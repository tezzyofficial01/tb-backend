...
// ✅ NEW IMPORT for sending notification
...
// 2️⃣ UPDATE USER BALANCE (Now with Notification)
async function updateUserBalance(req, res) {
  try {
    ...
    await sendNotification(user._id, type, Math.abs(amount));
...
// 4️⃣ TODAY'S ROUNDS SUMMARY (unchanged)
async function todayRoundsSummary(req, res) {
  try {
    ...
    const bets = await Bet.find({ createdAt: { $gte: startOfDay, $lte: nowIST } });
    ...
    for (let r = 1; r <= currentRoundNumber; r++) {
      ...
      rounds.push({ round: r, totalBet, winner, totalPayout });
    }
...
// 5️⃣ TODAY OVERALL SUMMARY (unchanged)
